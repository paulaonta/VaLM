...
import numpy as np
import faiss
#from faiss.resources import StandardGpuResources
import time
import logging
import sys
...
def train(dstore_mmap=None, dstore_size=None, dstore_fp16=True):
    ...
    logger = logging.getLogger("clip.image.retrieval.train_datastore_gpu")
    ...
    parser.add_argument('--num_keys_to_add_at_a_time', default=100000, type=int,
                        help='can only load a certain amount of data to memory at a time.')#100000
    ...
    if args.dstore_fp16:
        if args.load_multiple_files:
            ...
            vals = np.memmap(args.concat_file_path + '/vals.npy', dtype=np.int, mode='w+', shape=(concat_size, 1)) #np.int
    ...
    if not os.path.exists(args.faiss_index + ".trained"):
        ...
        logger.info(str(keys.shape))
        num_training_point = 10000000 if args.ncentroids > 100000 else 1000000 
        print("Number of training init points are".format(num_training_point))
